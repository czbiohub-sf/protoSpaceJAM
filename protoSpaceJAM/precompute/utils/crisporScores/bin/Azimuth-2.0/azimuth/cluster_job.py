...
def create(
    ...
    degrees,
    GP_likelihoods,
    adaboost_learning_rates=None,
    adaboost_num_estimators=None,
    adaboost_max_depths=None,
    adaboost_CV=False,
    exp_name=None,
    learn_options=None,
):
    ...
    if user == "fusi":
        ...
        raise Exception(
            "ensure you are using the right username, then add a clause here"
        )
    job.priority = "Normal"
    job.min_nodes = 1
    job.max_nodes = 5000
    ...
    if job.username == "REDMOND\\fusi":
        ...
        python = (
            r"\\fusi1\crispr\python.exe"  # this will not get copied, but used in-place
        )
        ...
        python_path = (
            r"\\fusi1\crispr\lib\site-packages\;\\jennl2\D$\Source\CRISPR\analysis"
        )
        home = r"\\fusi1\CLUSTER_HOME"
    ...
    for model in models:
        if model in ["L1", "L2", "linreg", "doench", "logregL1", "RandomForest", "SVC"]:
            for order in orders:
                ...
                t.command_line = (
                    python
                    + " cli_run_model.py %s --order %d --output-dir %s --exp-name %s"
                    % (model, order, tempdir, exp_name)
                )
                ...
                for learning_rate in adaboost_learning_rates:
                    for num_estimators in adaboost_num_estimators:
                        for max_depth in adaboost_max_depths:
                            ...
                            t.command_line = (
                                python
                                + " cli_run_model.py %s --order %d --output-dir %s --adaboost-learning-rate %f --adaboost-num-estimators %d --adaboost-max-depth %d --exp-name %s"
                                % (
                                    model,
                                    order,
                                    tempdir,
            ...
            for likelihood in GP_likelihoods:
                for degree in degrees:
                    ...
                    t.command_line = (
                        python
                        + " cli_run_model.py %s --order 1 --weighted-degree %s --output-dir %s --likelihood %s --exp-name %s"
                        % (model, degree, tempdir, likelihood, exp_name)
                    )
                    cluster_setup(i, python_path, home, t, work_dir, tempdir)
                    t.min_nodes = 1
                    # t.min_cores = 1
            ...
            t = WinHPCTask()
            t.task_name = "CRISPR_task"
            t.command_line = python + " cli_run_model.py %s --output-dir %s" % (
                model,
                tempdir,
            )
            cluster_setup(i, python_path, home, t, work_dir, tempdir)
