...
def extract_NDCG_for_fold(
    metrics, fold, i, predictions, truth, y_ground_truth, test, y_pred, learn_options
):
...
def cross_validate(
    y_all, feature_sets, learn_options=None, TEST=False, train_genes=None, CV=True
):
    """
    ...
    assert learn_options["method"] in allowed_methods, (
        "invalid method: %s" % learn_options["method"]
    )
    assert (
        learn_options["method"] == "linreg"
        and learn_options["penalty"] == "L2"
        or learn_options["weighted"] is None
    ...
    if learn_options["cv"] == "stratified":
        ...
        cv = sklearn.cross_validation.StratifiedKFold(
            gene_classes, n_folds=n_folds, shuffle=True, indices=True
        )
        ...
        if learn_options["num_genes_remove_train"] is not None:
            for i, (train, test) in enumerate(cv):
                ...
                print(
                    "# train/train after/before is %s, %s"
                    % (len(cv[i][0]), len(cv_i_orig[0]))
                )
                print(
                    "# test/test after/before is %s, %s"
    ...
    if num_proc > 1:
        ...
        for i, fold in enumerate(cv):
            train, test = fold
            print(
                "working on fold %d of %d, with %d train and %d test"
                % (i, len(cv), len(train), len(test))
            )
            if learn_options["method"] == "GPy":
                job = pool.apply_async(
                    models.GP.gp_on_fold,
                    args=(
                        feature_sets,
                        train,
                        test,
                ...
                job = pool.apply_async(
                    precomputed_gRNAs.utils.crisporScores.bin.fusiDoench.analysis.models.regression.linreg_on_fold,
                    args=(
                        feature_sets,
                        train,
                        test,
                ...
                job = pool.apply_async(
                    precomputed_gRNAs.utils.crisporScores.bin.fusiDoench.analysis.models.regression.logreg_on_fold,
                    args=(
                        feature_sets,
                        train,
                        test,
                ...
                job = pool.apply_async(
                    precomputed_gRNAs.utils.crisporScores.bin.fusiDoench.analysis.models.regression.ARDRegression_on_fold,
                    args=(
                        feature_sets,
                        train,
                        test,
                ...
                job = pool.apply_async(
                    precomputed_gRNAs.utils.crisporScores.bin.fusiDoench.analysis.models.DNN.DNN_on_fold,
                    args=(
                        feature_sets,
                        train,
                        test,
        ...
        for i, fold in enumerate(cv):
            ...
            if learn_options["method"] == "GPy":
                ...
                y_pred, m[i] = precopmuted_gRNAs.utils.crisporScores.bin.fusiDoench.analysis.models.regression.linreg_on_fold(
                    feature_sets,
                    train,
                    test,
                    y,
                ...
                y_pred, m[i] = precopmuted_gRNAs.utils.crisporScores.bin.fusiDoench.analysis.models.regression.logreg_on_fold(
                    feature_sets,
                    train,
                    test,
                    y,
                ...
                y_pred, m[i] = precopmuted_gRNAs.utils.crisporScores.bin.fusiDoench.analysis.models.regression.ARDRegression_on_fold(
                    feature_sets,
                    train,
                    test,
                    y,
                ...
                y_pred, m[i] = precomputed_gRNAs.utils.crisporScores.bin.fusiDoench.analysis.models.DNN.DNN_on_fold(
                    feature_sets,
                    train,
                    test,
                    y,
            ...
            print(
                "\t\tSpearman correlation: ", util.spearmanr_nonan(y[test], y_pred)[0]
            )
