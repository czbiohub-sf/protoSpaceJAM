...
def getOffTargetsNGG(
    ...
    if coreMismatches == "NA" or coreRange == "NA":
        command = (
            ...
            + " -n "
            + str(2 + 1)
        )  # plus 1 mismatch in the PAM, maximum 3!
        command = (
            command + " -l " + str(2 + len(revPAM))
        )  # minimum 5, additional 3 bases from the PAM
        ...
        command = (
            command + " -c " + revPAM + reverse_complement(sequence[: -len(revPAM)])
        )
    else:
        command = (
            bowtiePath
            ...
            + indexPath
            + " -n "
            + str(coreMismatches + 1)
        )  # plus 1 mismatch in the PAM, maximum 3!
        command = (
            command + " -l " + str(coreRange + len(revPAM))
        )  # minimum 5, additional 3 bases from the PAM
        command = command + " -e " + str(totalMismatches * 30 + 30)
        command = command + " -y "  # try hard to find all mismatched seeds
        command = command + " --quiet "  # just print alignments, no info
...
# Cpf1
def getOffTargetsTTTN(
    sequence, bowtiePath, indexPath, exons, genes, totalMismatches, PAM
):

    command = (
        bowtiePath + os.path.sep + "bowtie " + " -a " + indexPath + " -n " + str(2 + 1)
    )  # plus 1 mismatch in the PAM, maximum 3!
...
def getOffTargetsOtherPAMs(
    ...
    bowtieOutput = os.popen(
        ...
        + indexPath
        + " -n "
        + str(countWildcardInSeed)
        + " -l 5"
        + " -e "
        + str(totalMismatches * 30 + countWildcard * 30)
        + " -y "
        + " --quiet "  # try hard to find all mismatched seeds
        + " -c "  # just print alignments, no info
        + revPAM
        + reverse_complement(sequence[: -len(PAM)]),
        "r",
    )
    offtargets = []
    ...
    while True:
        ...
        if PAM == "NNGRRT":  # A**C**
            ...
            if (
                columns[1] == "-"
                and not (mismatches[0][2] in "AG" and mismatches[1][2] in "AG")
            ) or (
                columns[1] == "+"
                and not (mismatches[0][2] in "TC" and mismatches[1][2] in "TC")
            ):
                continue
...
def getOffTargets(
    ...
    if pamType == "NRG" or pamType == "NGG":
        ...
        if pamType == "NRG":
            offtargets = offtargets + getOffTargetsNGG(
                sequence,
                bowtiePath,
                indexPath,
                exons,
        ...
        offtargets = getOffTargetsTTTN(
            sequence, bowtiePath, indexPath, exons, genes, totalMismatches, pamType
        )
    else:
        offtargets = getOffTargetsOtherPAMs(
            sequence, bowtiePath, indexPath, exons, genes, totalMismatches, pamType
        )
...
class Offtarget:
    ...
    def __newFwd(
        ...
        for substitution in tmp:
            ...
            if (
                idx < lengthPAM
            ):  # The mismatch in the PAM is not considered for score calculation of alignment
                continue
    ...
    def addOffTargets(self, offTargets, coordinates, isThereGeneInfo):
        ...
        for idx in range(len(offTargets)):
            ...
            if not coordinates is None:
                ...
                if (
                    offTarget.chromosome == coordinates[0]
                    and offTarget.start >= coordinates[1]
                    and offTarget.start <= coordinates[2]
                ):
                    continue
        ...
        if totalMismatches > 0:
            self.score = (
                averMismatches / totalMismatches
                + averDistance / totalMismatches
                - totalMismatches
            )
        else:
            self.score = 100
...
class sgRNAbindingSites:
    ...
    def add(self, targetSeq, sequence, position, strand, fwdPrimer, revPrimer):
        newSite = sgRNAbindingSite(
            targetSeq, sequence, position, strand, fwdPrimer, revPrimer
        )
...
def getSeqCoords(seq, bowtiePath, indexPath):
    ...
    if len(seq) < 500:
        ...
        bowtieOutput = os.popen(
            bowtiePath + os.path.sep + "bowtie " + indexPath + " --quiet -c " + seq, "r"
        )
...
def addCandidateTargets(
    pam, target_size, sgRNA5, sgRNA3, query, strand, candidates, fwdPrimer, revPrimer
):
    ...
    if pam == "TTTN":
        ...
        for index in indices:
            if (index + target_size + len(pam)) > len(query):
                continue
            candidate_sequence = query[
                index + len(pam) : index + len(pam) + target_size
            ]
            pam_sequence = query[index : index + len(pam)]
            if (not re.search(sgRNA5_re, candidate_sequence) is None) and (
                not re.search(sgRNA3_re, candidate_sequence) is None
            ):
                # we need to transform the index from the reversed sequence to the forward sequence
                if strand == "+":
                    candidates.add(
                        candidate_sequence,
                        pam_sequence + candidate_sequence,
                        index,
                        strand,
                        fwdPrimer,
                        revPrimer,
                    ...
                    candidates.add(
                        candidate_sequence,
                        pam_sequence + candidate_sequence,
                        len(query) - (index + target_size + len(pam)),
                        strand,
                        fwdPrimer,
                        revPrimer,
                    )
        ...
        for index in indices:
            ...
            if (not re.search(sgRNA5_re, candidate_sequence) is None) and (
                ...
                if strand == "+":
                    candidates.add(
                        candidate_sequence,
                        candidate_sequence + pam_sequence,
                        index - target_size,
                        strand,
                        fwdPrimer,
                        revPrimer,
...
def doSearch(
    ...
    if exonsFile is not None and genesFile is not None:
        try:
            ...
            sys.stderr.write(
                "The bx-python module is not available. Ignoring exon and gene files!\n"
            )
    ...
    for idx in range(len(candidates.sites)):
        ...
        if maxOT < float("inf"):
            candidates.sites[idx].addOffTargets(
                ot[:maxOT], coordinates, len(exons.chroms) > 0
            )
        else:
        ...
        sys.stderr.write(
            "Sequence %s: Done with candidate %i out of %i.\n"
    ...
    if len(candidates.sites) > 1:
        for idx in range(len(candidates.sites)):
            if maxScore > minScore:
                candidates.sites[idx].score = (
                    (candidates.sites[idx].score - minScore)
                    / (maxScore - minScore)
                    * 1000
                )
            else:
    ...
    if coordinates is not None:
        for idx in range(len(candidates.sites)):
            bedFile.write(
                coordinates[0]
                + "\t"
                + str(coordinates[1] + candidates.sites[idx].position)
                + "\t"
                + str(coordinates[1] + candidates.sites[idx].position + totalSeqSize)
                + "\t"
                + candidates.sites[idx].label
                + "\t"
                + str(int(candidates.sites[idx].score))
                + "\t"
                + candidates.sites[idx].strand
                + "\n"
            )
    else:
        for idx in range(len(candidates.sites)):
            bedFile.write(
                name
                + "\t"
                + str(candidates.sites[idx].position)
                + "\t"
                + str(candidates.sites[idx].position + totalSeqSize)
                + "\t"
                + candidates.sites[idx].label
                + "\t"
                + str(int(candidates.sites[idx].score))
    ...
    for idx in range(0, len(candidates.sites)):
        ...
        if candidates.sites[idx].effi_score is None:
            ...
            output.write(
                ...
                + str(int(candidates.sites[idx].score))
                + "\tCRISPRater score\t"
                + getScoreText(candidates.sites[idx].effi_score)
                + "\n"
            )
        ...
        if candidates.sites[idx].oligo1 != "":
            ...
            output.write(
                "Oligo adding fwd\t" + str(candidates.sites[idx].oligoAfwd) + "\n"
            )
            output.write(
                "Oligo adding rev\t" + str(candidates.sites[idx].oligoArev) + "\n"
            )
            if (
                candidates.sites[idx].oligoSfwd != ""
                and candidates.sites[idx].oligoSrev != ""
            ):
                output.write(
                    "Oligo substituting fwd\t"
                    + str(candidates.sites[idx].oligoSfwd)
                    + "\n"
                )
                output.write(
                    "Oligo substituting rev\t"
                    + str(candidates.sites[idx].oligoSrev)
                    + "\n"
                )

        if pamType == "TTTN":
            output.write(
                "Chromosome\tstart\tend\tstrand\tMM\tPAM\ttarget_seq\talignment\tdistance\tposition\tgene name\tgene id\n"
            )
        else:
            output.write(
                "Chromosome\tstart\tend\tstrand\tMM\ttarget_seq\tPAM\talignment\tdistance\tposition\tgene name\tgene id\n"
            )
        for idx2 in range(0, len(candidates.sites[idx].offTargets)):
            ...
            if pamType == "TTTN":
                output.write(
                    "\t"
                    + offTarget.sequence[: len(pamType)]
                    + "\t"
                    + offTarget.sequence[len(pamType) :]
                )
            else:
            ...
            output.write(
                ...
                + offTarget.alignment
                + "\t"
                + str(offTarget.distance)
                + "\t"
                + getPlainOTPosition(offTarget.distance, offTarget.intragenic)
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        description="CCTop is the CRISPR/Cas9 Target online predictor.",
    ...
    parser.add_argument(
        ...
        help="Fasta file containing the sequence(s) to be scanned for sgRNA candidates.",
        required=True,
    )
    parser.add_argument(
        "--index",
        metavar="<file>",
        help="Path to the bowtie index files including the name of the index.",
        required=True,
    )
    parser.add_argument(
        "--bowtie",
        metavar="<folder>",
        help="Path to the folder where the executable bowtie is.",
        default="." + os.path.sep,
    )
    parser.add_argument(
        "--targetSize",
        metavar="<int>",
        help="Target site length. (default: %(default)s)",
        default=20,
        type=int,
    )
    parser.add_argument(
        "--pam",
        help="PAM type. (default: %(default)s)",
        default="NGG",
        choices=["NGG", "NRG", "TTTN", "NNGRRT", "NNNNGATT", "NNAGAAW", "NAAAAC"],
    )
    parser.add_argument(
        "--sgRNA5",
        metavar="<sequence>",
        type=valid_dinucleotideIUPAC,
        help="Filter candidates target sites with the most 5 prime nucleotides defined by this sequence. IUPAC code allowed. (default: %(default)s)",
        default="NN",
    )
    parser.add_argument(
        "--sgRNA3",
    ...
    parser.add_argument(
        ...
        metavar="<sequence>",
        type=valid_overhang,
        help="Sequence of the 5 prime forward cloning oligo. (default: %(default)s)",
        default="TAGG",
    )
    parser.add_argument(
        "--revOverhang",
        metavar="<sequence>",
        type=valid_overhang,
        help="Sequence of the 5 prime reverse cloning oligo. (default: %(default)s)",
        default="AAAC",
    )
    parser.add_argument(
        "--totalMM",
        metavar="<int>",
        help="Number of total maximum mismatches allowed in the off-target sites. (default: %(default)s)",
        default=4,
        type=int,
    )
    parser.add_argument(
        "--coreLength",
        metavar="<int>",
        help="Number of bases that enclose the core of the target site. (default: %(default)s)",
        default=12,
        type=int,
    )
    parser.add_argument(
        "--coreMM",
        metavar="<int>",
        help="Number of maximum mismatches allowed in the core of the off-target sites. (default: %(default)s)",
        default=2,
        type=int,
    )
    parser.add_argument(
        "--maxOT",
        metavar="<int>",
        help="Maximum number of off-target sites to be reported. (default: %(default)s)",
        default=float("inf"),
        type=int,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--exonsFile",
        metavar="<file>",
        help="Path to the pseudo-bed file containing the coordinate of exons in the target genome. (default: NotUsed)",
        default=None,
    )
    parser.add_argument(
        "--genesFile",
        metavar="<file>",
        help="Path to the pseudo-bed file containing the coordinate of genes in the target genome. (default: NotUsed)",
        default=None,
    )
    ...
    for line in lines:
        ...
        if len(line) > 1 and line[0] == ">":  # header
            if seqLine != "":
                ...
                sys.stderr.write(
                    "It looks that your input file is not (multi)fasta format. Please check it and try again."
                )
                sys.exit(1)
    ...
    if seqLine != "":
        ...
        sys.stderr.write(
            "It looks that you file is not (multi)fasta format. Please check it and try again."
        )
        sys.exit(1)
    ...
    for sequence in fileContent:
        doSearch(
            ...
            args.revOverhang,
            args.output,
            protoSpaceJAM.precompute.utils.FindOfftargetBwa.bin.Linux.bowtie,
            args.index,
            args.exonsFile,
            args.genesFile,
            args.maxOT,
