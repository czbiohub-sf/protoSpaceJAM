...
weights = pkl.load(open(os.path.join(
    Lindel.__path__[0], "Model_weights.pkl"), "rb"))
prerequesites = pkl.load(
    open(os.path.join(
        Lindel.__path__[0], "model_prereq.pkl"), "rb")
)
...
try:
    ...
    print(
        "Error: No PAM sequence is identified.Please check your sequence and try again"
    )
