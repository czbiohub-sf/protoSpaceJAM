...
class test_with_OpenCell_design(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        print("running protoSpaceJAM SNP mode using seven designs (quick SNP test)...")
        ...
        if os.path.exists(os.path.join("tests", "quick_SNP_test_result")):
            shutil.rmtree(os.path.join("tests", "quick_SNP_test_result"))

        #run protoSpaceJAM
        test_args = {
                    "path2csv": os.path.join("input", "test_input.csv"),
                    "outdir": os.path.join("tests","quick_SNP_test_result"),
                    "ssODN_max_size": 200,
                    "payload_type": "SNP",
                    "SNPpayload": "AA",
    ...
    def tearDownClass(cls):
        keep_results = True
        #Remove results
        if (not keep_results) and os.path.exists(os.path.join("tests", "quick_SNP_test_result")):
            shutil.rmtree(os.path.join("tests", "quick_SNP_test_result"))
        #pass    #keep results

    def test_if_generated_results(self):
        #check if results were generated
        self.assertTrue(os.path.isfile(os.path.join("tests", "quick_SNP_test_result", "result.csv")))
    ...
    def test_compare_results(self):
        ...
        ExpectedResPath = os.path.join("tests","GroundTruths", "quick_SNP_result.csv")
        NewResPath = os.path.join("tests","quick_SNP_test_result","result.csv")
