...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="all",
        type=str,
        help="this parameter decides which file to load, the files have name start/stop_gRNAs_of_{num_to_process}_genes.pickle",
        metavar="",
    )
...
def main():
    try:
        ...
        start_gRNA_df = read_pickle_files(
            os.path.join(f"../pickles/start_gRNAs_of_{num_to_process}_genes.pickle")
        )
        stop_gRNA_df = read_pickle_files(
            os.path.join(f"pickles/stop_gRNAs_of_{num_to_process}_genes.pickle")
        )

        best_start_gRNA_df = read_pickle_files(
            os.path.join(
                f"../pickles/best_start_gRNAs_of_{num_to_process}_genes.pickle"
            )
        )
        best_stop_gRNA_df = read_pickle_files(
            os.path.join(f"../pickles/best_stop_gRNAs_of_{num_to_process}_genes.pickle")
        )

        start_failed = read_pickle_files(
            os.path.join(
                f"../pickles/start_failed_IDs_of_{num_to_process}_genes.pickle"
            )
        )
        stop_failed = read_pickle_files(
            os.path.join(f"../pickles/stop_failed_IDs_of_{num_to_process}_genes.pickle")
        )
        ...
        plot_hist(
            df=df2plot, col="pos_weight", bin_num=bin_num, num_to_process=num_to_process
        )
        plot_hist(
            df=df2plot,
            col="spec_weight",
...
def plot_hist(df, col, bin_num, num_to_process):
    ...
    if name == "final_weight":
        name = "final_score"
    plt.title(
        f"Distribution of {name} for {data_volume} gRNAs in {num_to_process} transcripts",
        fontsize=13,
    )
