...
def search_gRNA(protosp_len, PAM, search_in, flanksize=100):
    ...
    gPAM = PAM
    gPAM = re.sub("N", "[ACGT]", gPAM, flags=re.IGNORECASE) #IUPAC nucleotide code http://www.bioinformatics.org/sms/iupac.html
    ...
    gRNA_regex = re.compile(r"(?=(.{" + re.escape(str(protosp_len)) + r"})" + r"(" + gPAM + r"))", re.IGNORECASE) #use lookahead assertion (?=...) to find overlapping gRNAs
    # print(gRNA_regex)
    gRNA_list = []
    ...
    for m in gRNA_regex.finditer(search_in):
        if m:
            if len(m.groups())>=2:
                ...
                lf = get_leftflank(seq=search_in, pos=st, flanksize=flanksize) # pos always is in respect to the 5'
                rf = get_rightflank(seq=search_in, pos=end, flanksize=flanksize) # pos always is in respect to the 5'

                gRNA_list.append(gRNA(name = f"gRNA_+_{st}_{end}",
                                      protospacer = protosp, pam = gPAM,
                                      g_strand="+",
                                      g_st = st, g_en = end,
    ...
    for m in gRNA_regex.finditer(search_in_revcom):
        if m:
            if len(m.groups())>=2:
                ...
                lf = get_leftflank(seq=search_in_revcom, pos=st+1, flanksize=flanksize) # pos always is in respect to the 5'. We need flanks on the - strand, thus before changing to indexing from the 5' of the + strand
                rf = get_rightflank(seq=search_in_revcom, pos=end, flanksize=flanksize)

                st = seq_len - st  #change to indexing from the 5' of the + strand, and also change from 0-index to 1-indexed
                end = seq_len - end + 1 #change to indexing from the 5' of the + strand,


                gRNA_list.append(gRNA(name = f"gRNA_-_{st}_{end}",
                                        protospacer = protosp, pam = gPAM,
                                        g_strand="-",
                                        g_st = st, g_en = end, prsp_len = len(protosp),
                                        protospacer5p_flank = lf,
                                        pam3p_flank = rf))
...
def get_rightflank(seq, pos, flanksize):
    ...
    if flanksize<0 or pos<0:
        raise ValueError(f"flanksize must >=0 and pos must >=0; flanksize is set to {flanksize}, pos is set to {pos}")
    if pos>len(seq):
...
def search_gRNA_in_cdsList(cdsList, my_transcript, cds_flank_len, PAM, protosp_len, adjust_frame, HDR_flank_len):
    """
