...
def fetch_ensembl_transcript(
    ensembl_transcript_id, exon_annot=False, is_retry: bool = False
):
    ...
    try:
        ...
        log.error(
            "Ensembl sequence REST query returned error " "{}".format(response.text)
        )
    ...
    try:
        ...
        log.info(
            f"Retrieved sequence {response_data['desc']} of length "
    ...
    if exon_annot:
        ...
        try:
            ...
            num_exon_boundaries = len([f for f in record.features if f.type == "exon"])

            num_cds_boundaries = len([f for f in record.features if f.type == "cds"])

            log.info(
                f"Retrieved {num_exon_boundaries} exons and "
