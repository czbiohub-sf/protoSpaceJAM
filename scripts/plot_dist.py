...
def main():
    try:
        ...
        df["gstrand"] = [
            -1 if i > 0 else 1 for i in (df["gRNA_start"] - df["gRNA_end"])
        ]
        ...
        # plot histogram
        plot_hist(
            lst=df_N["distance_between_cut_and_edit"].values,
            label="distance_between_cut_and_edit (N term)",
            bin_num=bin_num,
        )
        plot_hist(
            lst=df_C["distance_between_cut_and_edit"].values,
            label="distance_between_cut_and_edit (C term)",
            bin_num=bin_num,
        )
