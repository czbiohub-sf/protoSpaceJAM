...
def parse_args():
    ...
    parser.add_argument(
        "--gRNA_count", default="", type=int, help="path to the gzfile", metavar=""
    )
    parser.add_argument(
        "--genome_fa",
        default="",
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="num of threads to use for bwa",
        metavar="",
    )
...
def main():
    try:
        ...
        total_chunk_count = str(
            math.ceil(int(str(config["gRNA_count"]).strip()) / n_gRNA_per_chunk)
        )
        ...
        with gzip.open(file, "rt") as fh, gzip.open(
            os.path.join(outdir_path, f"{config['gzfile'].rstrip('.gz')}.mapped.gz"),
            "wt",
        ) as wgfh:
            wfh = open(f"{file}.{n_chunk}.fa", "w")
            wgfh_tmp = gzip.open(
                os.path.join(outdir_path, f"{config['gzfile'].rstrip('.gz')}.tmp.gz"),
                "wt",
            )
            for line in fh:
                # print(line)
                ...
                if file_gRNA_count % n_gRNA_per_chunk == 0:
                    ...
                    with open(f"{fastafile}_bwa/bwa.out.bed", "r") as BEDOUT:
                        for line in BEDOUT:
                            ...
                            if gRNAname in Bwa_mapping.keys():
                                Bwa_mapping[gRNAname].append(
                                    f"{matchChr}:{matchStart}-{matchEnd}_{matchInfo}"
                                )
                            else:
                                Bwa_mapping[gRNAname] = [
                                    f"{matchChr}:{matchStart}-{matchEnd}_{matchInfo}"
                                ]
                    ...
                    print(
                        f"Finished processing chunk {n_chunk} (total {total_chunk_count}), chunk size:{n_gRNA_per_chunk}, took {elapsed_min_chunk:.2f} min",
                        flush=True,
                    )
                    ...
                    if int(n_chunk) < int(
                        total_chunk_count
                    ):  # n_chunk is 0-indexed, and total_chunk_count is 1-indexed, so n_chunk should never reach total_chunk_count
        ...
        log.info(
            f"finished in {elapsed_min:.2f} min, bwa-mapped {gRNA_count} gRNAs in file {file}"
        )
        print(
            f"finished in {elapsed_min:.2f} min, bwa-mapped {gRNA_count} gRNAs in file {file}",
            flush=True,
