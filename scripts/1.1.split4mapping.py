...
def main():
    try:
        #check input
        if (config["dir"] is None or config["dir"] == "") or (config["tab"] is None or config["tab"] == ""):
        ...
        if (not "Chr" in df.columns) or (not "gRNA_count" in df.columns):
            log.error(f"The csv file does not contain either a column named: Chr or gRNA_count")
            sys.exit("Please fix the error(s) above and rerun the script")
        ...
        with open(os.path.join(new_tab), "w") as wfh:
            ...
            for index, row in df.iterrows():
                ...
                if tab_gRNA_count <= part_size:
                    shutil.copy2(os.path.join(config["dir"], f"{current_chr}.tab.gz"), os.path.join(path2dir, f"{current_chr}.tab.gz"))
                ...
                #chrs that need to be split
                    ...
                    with gzip.open(os.path.join(config["dir"], f"{current_chr}.tab.gz"), "rt") as gzfh:
                        part = 0
                        file_gRNA_count =0
                        part_gRNA_count =0
        ...
        log.info(f"finished in {elapsed_min:.2f} min, processed {chr_count} chromosomes, {gRNA_count} gRNAs")
        print(f"finished in {elapsed_min:.2f} min, processed {chr_count} chromosomes, {gRNA_count} gRNAs")
