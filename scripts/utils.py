...
def get_HDR_arms(loc, half_len, type,genome_ver):
    ...
    if type == "start":
        if Strand == 1:
            vanilla_left_arm = get_seq(chr = Chr, start = Pos-half_len+1, end = Pos+1, strand = 1, genome_ver = genome_ver)
            vanilla_right_arm = get_seq(chr = Chr, start = Pos+1, end = Pos+half_len+1, strand = 1, genome_ver = genome_ver)
            return([vanilla_left_arm,vanilla_right_arm, Pos-half_len+1,Pos,Pos+1,Pos+half_len])
        elif Strand == -1:
            vanilla_left_arm = get_seq(chr = Chr, start = Pos-half_len, end = Pos, strand = 1, genome_ver = genome_ver)
            vanilla_right_arm = get_seq(chr = Chr, start = Pos, end = Pos+half_len, strand = 1, genome_ver = genome_ver)
...
def get_cut_pos(start,strand):
    ...
    if strand == "+" or strand == "1" or strand == 1:
        ...
        cutPos = start - 16 -1 # -1 because we want the cutsite to be behind the pos (viewed in the +1 strand)
    return(cutPos)
