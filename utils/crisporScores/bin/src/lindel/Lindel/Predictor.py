...
def onehotencoder(seq):
    ...
    for j in range(l):
        encode[head_idx[seq[j] + str(j)]] = 1.0
    for k in range(l - 1):
...
def gen_prediction(seq, wb, prereq):
    ...
    if seq[33:36] not in pam:
        ...
        raise Exception(
            "Error for guide %s: the PAM %s (pos 33-36 in input seq) is not supported by Lindel."
            % (guide, seq[33:36])
        )
    ...
    input_del = np.concatenate(
        (create_feature_array(features, indels), input_indel), axis=None
    )
...
def write_json(seq, array, freq):
    ...
    output = [
        {"Sequence": s, "Frequency": f, "Indels": i}
