...
def featurize_data(data, learn_options, Y, gene_position):
    ...
    if learn_options["include_gene_effect"]:
        ...
        feature_sets["gene effect"] = pandas.DataFrame(
            one_hot_genes,
            columns=["gene_%d" % i for i in range(one_hot_genes.shape[1])],
    ...
    if learn_options["include_strand"]:
        feature_sets["Strand effect"] = (
            pandas.DataFrame(data["Strand"]) == "sense"
        ) * 1
...
def get_all_order_nuc_features(
    data, feature_sets, learn_options, maxorder, max_index_to_use, prefix=""
):
...
def get_micro_homology_features(gene_names, learn_options, X):
    ...
    if True:
        ...
        for gene in gene_names.unique():
            ...
            print(
                "getting microhomology for all %d guides in gene %s"
                % (len(guide_inds), gene)
            )
...
def local_gene_seq_features(gene_names, learn_options, X):
    ...
    for gene in gene_names.unique():
        ...
        for ps in np.where(gene_names.values == gene)[0]:
            ...
            assert len(left_win) == len(right_win), (
                "k_mer_context, %s, is too large" % k_mer_length
            )
...
def apply_nucleotide_features(
    ...
    if fast:
        ...
        for s in seq_data_frame.values:
            ...
            feat_pd_tmp, feat_pi_tmp = nucleotide_features(
                s, order, include_pos_independent, max_index_to_use, prefix=prefix
            )
...
def nucleotide_features(
    s, order, include_pos_independent, max_index_to_use, prefix="", feature_type="all"
):
    """
    ...
    index_dependent = [
        "%s_pd.Order%d_P%d" % (prefix, order, i)
    ...
    if feature_type == "all" or feature_type == "pos_independent":
        if include_pos_independent:
            index_independent = [
                "%s_pi.Order%d_P%d" % (prefix, order, i)
                for i in range(len(features_pos_independent))
            ]
            if feature_type == "all":
                return (
                    pandas.Series(features_pos_dependent, index=index_dependent),
                    pandas.Series(features_pos_independent, index=index_independent),
                )
            else:
