...
def concatenate_feature_sets(feature_sets):
    ...
    for set in list(feature_sets.keys()):
        ...
        assert F == F2, "not same # individuals for features %s and %s" % (
            list(feature_sets.keys())[0],
            set,
        )
...
def get_ranks(y, thresh=0.8, prefix="", flip=False, col_name="score"):
    ...
    y_quantized = pandas.DataFrame(
        pandas.qcut(y[col_name], 5, labels=np.arange(5.0)).astype(float)
    )  # quantized vector
