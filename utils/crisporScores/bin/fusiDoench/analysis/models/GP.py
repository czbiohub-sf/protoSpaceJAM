...
def gp_on_fold(feature_sets, train, test, y, y_all, inputs, dim, dimsum, learn_options):
    ...
    if "drug" in feature_sets:
        ...
        kern += GPy.kern.Linear(
            Q, active_dims=list(range(current_dim, current_dim + Q)), name="drug_lin"
        )
    ...
    if "gene effect" in feature_sets:
        ...
        kern += GPy.kern.Linear(
            Q, active_dims=list(range(current_dim, current_dim + Q)), name="gene_lin"
        )
    ...
    if "Percent Peptide" in feature_sets:
        ...
        kern += GPy.kern.RBF(
            Q,
            active_dims=list(range(current_dim, current_dim + Q)),
            name="percent_pept",
        )
    ...
    if "Nucleotide cut position" in feature_sets:
        ...
        kern += GPy.kern.RBF(
            Q, active_dims=list(range(current_dim, current_dim + Q)), name="nucleo_cut"
        )
    ...
    if "Strand effect" in feature_sets:
        ...
        kern += GPy.kern.Linear(
            Q, active_dims=list(range(current_dim, current_dim + Q)), name="strand"
        )
    ...
    if "NGGX" in feature_sets:
        ...
        kern += GPy.kern.Linear(
            Q, active_dims=list(range(current_dim, current_dim + Q)), name="NGGX"
        )
