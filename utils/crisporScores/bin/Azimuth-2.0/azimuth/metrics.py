...
def ndcg_at_k_ties(
    labels, predictions, k, method=0, normalize_from_below_too=False, theta=None
):
    """
...
if __name__ == "__main__":
    ...
    for t in range(T):
        ...
        for i, w in enumerate(weights):
            ...
            (
                ...
                corr1,
                corr2,
            ) = elevation.spearman_weighted_swap_perm_test(
    ...
    for i, theta in enumerate(theta_range.tolist()):
        ...
        mytitle = (
            "Norm. hist p-values Steiger w weighted Spearman\n %d null samples, N=%d, weight=%s"
            % (T, N, str(theta))
        )
        ut.qqplotp(
            allp[i, :],
    ...
    print(
        (
            "ndcg1=%f, ndcg2=%f, ndcg_diff=%f, p=%f"
            % (ndcg1, ndcg2, real_ndcg_diff, pval)
        )
    )
