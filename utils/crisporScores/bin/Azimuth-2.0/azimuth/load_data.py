...
def from_custom_file(data_file, learn_options):
    ...
    for col in mandatory_columns:
        assert col in data.columns, (
            "inputs for prediction must include these columns: %s" % mandatory_columns
        )
...
def from_file(data_file, learn_options, data_file2=None, data_file3=None):
    if learn_options["V"] == 1:  # from Nature Biotech paper
        ...
        annotations, gene_position, target_genes, Xdf, Y = read_V1_data(
        ...
        Xdf, drugs_to_genes, target_genes, Y, gene_position = read_V2_data(
            data_file, learn_options
        )
        ...
        Xdf, Y, gene_position, target_genes = mergeV1_V2(
        ...
        Xdf, Y, gene_position, target_genes = merge_all(
            data_file, data_file2, data_file3, learn_options
        )
...
def read_V1_data(
    data_file, learn_options, AML_file=cur_dir + "/data/V1_suppl_data.txt"
):
    ...
    gene_position = pandas.merge(
        Xdf, annotations, how="inner", left_index=True, right_index=True
    )
    ...
    Y[
        "test"
    ] = 1  # for bookeeping to keep consistent with V2 which uses this for "extra pairs"
...
def read_xu_et_al(data_file, learn_options=None, verbose=True, subsetting="ours"):
    ...
    for d in datasets:
        data_efficient = pandas.read_excel(
            data_file, sheetname="%s_efficient_sgRNA" % d, skiprows=2
        )
        data_inefficient = pandas.read_excel(
            data_file, sheetname="%s_inefficient_sgRNA" % d, skiprows=2
        )

        data_efficient["threshold"] = 1.0
        data_inefficient["threshold"] = 0.0

        exp_data = pandas.concat((data_efficient, data_inefficient))
        exp_data["rank_KBM7"] = exp_data.groupby("Gene Symbol")[
            "log2 fold change, KBM7"
        ].transform(rank_transform)
        exp_data["rank_HL60"] = exp_data.groupby("Gene Symbol")[
            "log2 fold change, HL60"
        ].transform(rank_transform)
    ...
    if subsetting == "ours":
        aggregated["sequence(target+3'+5')"] = aggregated[
            "sequence(target+3'+5')"
        ].apply(lambda x: x[6:-4])
    else:
        aggregated["sequence(target+3'+5')"] = aggregated[
            "sequence(target+3'+5')"
        ].apply(lambda x: x[10:])
    ...
    aggregated["sequence(target+3'+5')"] = aggregated["sequence(target+3'+5')"].apply(
        lambda x: x.upper()
    )
    ...
    aggregated.rename(
        columns={
            "sequence(target+3'+5')": "30mer",
            "Gene Symbol": "Target gene",
            "strand": "Strand",
        },
        inplace=True,
    )
    ...
    return (
        df[["Nucleotide cut position", "Percent Peptide", "Amino Acid Cut position"]],
        target_genes,
        df[["30mer", "Strand"]],
        df[
            ["score_drug_gene_rank", "score_drug_gene_threshold", "test", "Target gene"]
        ],
    )
...
def read_V2_data(data_file, learn_options=None, verbose=True):
    ...
    for drug in list(drugs_to_genes.keys()):
        ...
        for g in genes:
            ...
            if verbose:
                print(
                    "Loaded %d samples for gene %s \ttotal number of samples: %d"
    ...
    target = pandas.DataFrame(
        Xdf.index.get_level_values("Target gene").values,
        index=Y.index,
        columns=["Target gene"],
    )
    ...
    for drug in list(drugs_to_genes.keys()):
        ...
        for gene in gene_list:
            ytmp = pandas.DataFrame(
                Y.xs((gene, drug), level=["Target gene", "drug"], drop_level=False)[
                    "score"
                ]
            )
    ...
    if PLOT:
        ...
        labels = [
            ...
            "score_drug_threshold",
        ]

        for label in labels:
            plt.figure()
            plt.plot(Xdf["sgRNA Score"].values, Y[label].values, ".")
            r, pearp = sp.stats.pearsonr(
                Xdf["sgRNA Score"].values.flatten(), Y[label].values.flatten()
            )
...
def merge_all(data_file=None, data_file2=None, data_file3=None, learn_options=None):
    ...
    gene_position_xu, target_genes_xu, Xdf_xu, Y_xu = read_xu_et_al(
        data_file3, learn_options
    )
...
def mergeV1_V2(data_file, data_file2, learn_options):
    ...
    annotations, gene_position1, target_genes1, Xdf1, Y1 = read_V1_data(
        data_file, learn_options
    )
    ...
    if save_to_file:
        ...
        alldupind = np.where(
            Y.index.get_level_values(0).values == Y.index[onedupind][0]
        )[0]
        ...
        gene_position_tmp.index = pandas.MultiIndex.from_tuples(
            newindex, names=Y.index.names
        )
        XandY = pandas.merge(
            XandY, gene_position_tmp, how="inner", left_index=True, right_index=True
        )
