...
def convert_to_thirty_one(guide_seq, gene, strand):
    ...
    if ind == -1:
        print(
            "returning sequence+'A', could not find guide %s in gene %s"
            % (guide_seq, gene)
        )
        return gene_seq + "A"
    assert gene_seq[ind : (ind + len(guide_seq))] == guide_seq, "match not right"
...
def concatenate_feature_sets(feature_sets, keys=None):
    ...
    if False:
        inputs.shape
        for j in keys:
            print(j + str(feature_sets[j].shape))
        import ipdb
...
def get_ranks(y, thresh=0.8, prefix="", flip=False, col_name="score"):
    ...
    if flip:
        y_rank = (
            1.0 - y_rank
        )  # before this line, 1-labels where associated with low ranks, this flips it around (hence the y_rank > thresh below)
...
if __name__ == "__main__":
    ...
    if V == "1":
        human_data = pandas.read_excel(
            "data/V1_data.xlsx", sheetname=0, index_col=[0, 1]
        )
        mouse_data = pandas.read_excel(
            "data/V1_data.xlsx", sheetname=1, index_col=[0, 1]
        )
